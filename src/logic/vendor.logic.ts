 ...
 export const calculateContractStats = (contracts: any[], today: Date, suppliers: any[]) => {
    ...
    const expiringContracts = contractsWithEndDate.filter(c =>
      c.contractEndDate > today &&
      c.contractEndDate <= thirtyDaysFromNow
    );

    const expiredContracts = contractsWithEndDate.filter(c =>
      c.contractEndDate < today
    );
    ...
    function daysExpired(endDate: Date): number {
      return Math.ceil((today.getTime() - endDate.getTime()) / (1000 * 60 * 60 * 24));
    }
  };
  // ========== CALCULATE COMPLIANCE OVERVIEW ==========
 export const calculateComplianceOverview=(suppliers: any[]) => {
 ...
 export const calculateComplianceGauge=(suppliers: any[], assessments: any[], problems: any[], documents: any[], sendedAlertNotifications: any[]) => {
 ...
 export const calculateChartsData = (assessments: any[], problems: any[], suppliers: any[], thirtyDaysAgo: Date) => {
  ...
  export const calculateRecentUpdates = (assessments: any[], problems: any[], documents: any[], contracts: any[], suppliers: any[]) => {
    const updates: any[] = [];
 ...
 export const calculateNIS2Compliance = (suppliers: any[], today: Date, yesterday: Date) => {
  ...
  export const calculateSupplierStats = (suppliers: any[], vendorId: string, thirtyDaysAgo: Date) => {
  ...
  export const calculateAssessmentStats = (assessments: any[], suppliers: any[]) => {
  ...
  export const calculateProblemStats = (problems: any[]) => {
   ...
   export const   generateSupplierRecommendations=(supplier: any, categoryScores: any[], evidenceCompletionRate: number): string[] => {
