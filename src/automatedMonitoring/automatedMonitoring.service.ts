...
export class MonitoringQueueService {
   ...
   */
  private async hasEmailBeenSentToday(
    userId: string,
  ...
  private async sendRiskNotifications(supplier: any) {
    try {
      ...
      if (supplier.vendor?.user?.id) {
        ...
        const emailAlreadySent = await this.hasEmailBeenSentToday(
          supplier.vendor.user.id,
          'RISK_ALERT',
          supplier.id,
          supplier.vendorId
        );
        ...
        if (!emailAlreadySent) {
          ...
          if (notification) {
            await this.markEmailAsSent(notification.id, supplier.id, supplier.vendorId);
          }
        } else {
      ...
      if (supplier.user?.id) {
        ...
        if (!emailAlreadySent) {
          ...
          if (notification) {
            await this.markEmailAsSent(notification.id, supplier.id);
          }
        } else {
  ...
  private async processExpiringContracts(data: any) {
    ...
    try {
      ...
      for (const supplier of expiredContracts) {
        await this.sendContractExpiredNotifications(supplier);
      }
  ...
  private async sendContractExpiredNotifications(supplier: any) {
    try {
      // Send to vendor
      if (supplier.vendor?.user?.id) {
        // Check if email already sent today
      ...
      if (supplier.user?.id) {
        ...
        if (!emailAlreadySent) {
          const notification = await NotificationService.createNotification({
            ...
            message: `Your contract with ${supplier.vendor?.companyName} has expired. Please contact them for renewal.`,
            type: 'CONTRACT_EXPIRY',
            metadata: {
              supplierId: supplier.id,
      ...
      if (supplier.user?.id) {
        ...
        if (!emailAlreadySent) {
          const notification = await NotificationService.createNotification({
            ...
            message: `Your contract with ${supplier.vendor?.companyName} expires in ${daysLeft} days.`,
            type: notificationType,
            metadata: {
              supplierId: supplier.id,
              vendorName: supplier.vendor?.companyName,
  ...
  private shouldSendContractNotification(daysLeft: number): boolean {
  ...
  private async sendAssessmentReminders(supplier: any) {
    try {
      ...
      const incompleteCount = supplier.assessmentSubmissions?.length || 0;
      ...
      if (supplier.vendor?.user?.id) {
        ...
        const overdueAssessments = supplier.assessmentSubmissions?.filter(
          (sub: any) => new Date(sub.createdAt) < sevenDaysAgo
        ) || [];
