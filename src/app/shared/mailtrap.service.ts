...
export class MailtrapService {
  ...
  async sendHtmlEmail(options: SendEmailOptions): Promise<any> {
    try {
      ...
      throw new ApiError(500, `Failed to send HTML email: ${error.message}`);
    }
  }
   ...
   */
  async sendSupplierInvitationEmail(data: {
    ...
    const registrationUrl = `${config.APP.WEBSITE}/supplier/register?token=${data.invitationToken}`;
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background-color: #fff; padding: 30px; border-radius: 0 0 10px 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <p style="color: #666; font-size: 14px; margin-bottom: 5px;">
            <strong>Need help?</strong> Contact our support team at <a href="mailto:${config.APP.SUPPORT_EMAIL}" style="color: #667eea;">${config.APP.SUPPORT_EMAIL}</a>
          </p>
        ...
        <div style="text-align: center; margin-top: 20px; color: #999; font-size: 12px;">
          ...
          <p><a href="${config.APP.WEBSITE}" style="color: #667eea;">Visit our website</a></p>
        </div>
      </body>
      </html>
    `;
  ...
  async sendWelcomeEmail(data: {
    ...
    const dashboardUrl = `${config.APP.WEBSITE}/dashboard`;
    ...
    const htmlContent = `
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background-color: #fff; padding: 40px; border-radius: 0 0 10px 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <div style="display: flex; justify-content: space-between; margin-top: 30px;">
            <div style="text-align: center; flex: 1;">
              ...
              <p style="margin: 5px 0 0; font-size: 14px;"><a href="mailto:${config.APP.SUPPORT_EMAIL}" style="color: #667eea;">${config.APP.SUPPORT_EMAIL}</a></p>
            </div>
            <div style="text-align: center; flex: 1;">
              <div style="background-color: #667eea; color: white; width: 40px; height: 40px; line-height: 40px; border-radius: 50%; margin: 0 auto 10px; font-size: 20px;">📖</div>
              <p style="margin: 0; font-size: 12px; color: #666;">Documentation</p>
              <p style="margin: 5px 0 0; font-size: 14px;"><a href="${config.APP.WEBSITE}/docs" style="color: #667eea;">View Docs</a></p>
            </div>
            <div style="text-align: center; flex: 1;">
              <div style="background-color: #667eea; color: white; width: 40px; height: 40px; line-height: 40px; border-radius: 50%; margin: 0 auto 10px; font-size: 20px;">🎓</div>
              <p style="margin: 0; font-size: 12px; color: #666;">Tutorials</p>
              <p style="margin: 5px 0 0; font-size: 14px;"><a href="${config.APP.WEBSITE}/tutorials" style="color: #667eea;">Learn More</a></p>
            </div>
          </div>
        </div>
        ...
        <div style="text-align: center; margin-top: 20px; color: #999; font-size: 12px;">
          <p>© ${new Date().getFullYear()} CyberNark. All rights reserved.</p>
          <p><a href="${config.APP.WEBSITE}" style="color: #667eea;">Visit our website</a> | <a href="${config.APP.WEBSITE}/privacy" style="color: #667eea;">Privacy Policy</a></p>
        </div>
      </body>
      </html>
    `;
  ...
  async sendPasswordResetEmail(data: {
    ...
    const resetUrl = `${config.APP.WEBSITE}/reset-password?token=${data.resetToken}`;
    ...
    const htmlContent = `
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="text-align: center; margin-top: 20px; color: #999; font-size: 12px;">
          <p>© ${new Date().getFullYear()} CyberNark. All rights reserved.</p>
          <p><a href="${config.APP.WEBSITE}" style="color: #667eea;">Visit our website</a> | <a href="${config.APP.WEBSITE}/security" style="color: #667eea;">Security Center</a></p>
        </div>
      </body>
      </html>
    `;
  ...
  async testEmailConnection(): Promise<any> {
    try {
      const testResponse = await this.sendHtmlEmail({
        to: "test@example.com",
